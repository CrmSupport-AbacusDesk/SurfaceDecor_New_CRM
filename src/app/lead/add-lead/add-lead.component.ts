...
export class AddLeadComponent implements OnInit {
  ...
  getItemsList(search)
  {
    ...
    {
      console.log(i);
      this.tmpsearch=this.tmp_userList[i]['name'] != '' && this.tmp_userList[i]['name'] != null ? this.tmp_userList[i]['name'].toLowerCase():'';
  ...
  salesUserLIst(){
    this.serve.fetchData({'state':this.data.state?this.data.state:0},"User/sales_user_list").subscribe((result=>{
      console.log(result);
  ...
  show_country_error(){
    this.dialog.error('Please Select Correct Country');
  }
}
