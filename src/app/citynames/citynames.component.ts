...
@Component({
  selector: 'app-citynames',
  templateUrl: './citynames.component.html',
  styleUrls: ['./citynames.component.scss']
})
export class CitynamesComponent implements OnInit {
  skelton:any={}
  state='';
  district='';
  city='';
  ...
  goTo(date_from,date_to,state,district,city){

     console.log(date_from,date_to);
    console.log(this.data.data_type);
    console.log(this.data.user_type);
    console.log(date_from,date_to);
    this.rout.navigate(['/companynames', { 'start_date': date_from,'end_date': date_to,'data_type':this.data.data_type,'user_type':this.data.user_type,'state':state,'district':district,'city':city }]);
  ...
  this.rout.navigate(['/districtname', { 'start_date': this.searchData.start_date,'end_date': this.searchData.end_date,'data_type':this.searchData.data_type,'user_type':this.searchData.user_type,'state':this.searchData.state}]);
  ...
  get_report_data()
  {
    ...
    this.loader = true;
    this.db.fetchData({"search":this.data,'state':this.state,'district':this.district},"Travel/location_wise2")
    .subscribe(resp=>{
      console.log(resp);
  ...
  exportAsXLSX():void
  {
    ...
    {      
      this.excel_data.push({'city':this.report_data[i].city});
    ...
    this.db.exportAsExcelFile(this.excel_data,'CityWise Sales Report');
    this.loader = false;
  }
